...
CREATE PROCEDURE [dbo].[sp_SFMC_Header_Bkp_PreTMConversion]
as
...
IF OBJECT_ID('tempdb..#m2') IS NOT NULL
    DROP TABLE #m2
CREATE TABLE #m2 (ContactID varchar(64), PacID varchar(100), AdvanceID varchar(50), EmailAddress varchar(254)) -- multiple ContactID records
...
IF OBJECT_ID('tempdb..#c2') IS NOT NULL
    DROP TABLE #c2
CREATE TABLE #c2 (ContactID varchar(64), PacID varchar(100), AdvanceID varchar(50), EmailAddress varchar(254)) -- multiple ContactID records
...
insert into #m2 (ContactID, PacID, AdvanceID, EmailAddress)
SELECT a.ContactID, a.PacID, a.AdvanceID, a.EmailAddress 
FROM (
SELECT *
, ROW_NUMBER() OVER(PARTITION BY ContactID ORDER BY ISNULL(AdvanceID,'9999999999999999'), PacID) RowRank
From #m
) a
WHERE a.RowRank <= 6
...
insert into #c2 (ContactID, PacID, AdvanceID, EmailAddress)
SELECT a.ContactID, a.PacID, a.AdvanceID, a.EmailAddress 
FROM (
SELECT *
, ROW_NUMBER() OVER(PARTITION BY ContactID ORDER BY ISNULL(AdvanceID,'9999999999999999'), PacID) RowRank
