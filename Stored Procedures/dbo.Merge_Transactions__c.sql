...
DCH on 2016-10-25:	-	delete any transaction older than three years
...
DECLARE @DeleteDateThreshold	datetime;

SET @DeleteDateThreshold = (select convert(date,DATEADD(YEAR,-3,GETDATE())));



merge into dbo.Transactions__c as target
using  src.Transactions__c as SOURCE 
...
when matched and (
	...
	OR target.Order_Date__c < @DeleteDateThreshold
...
,target.changetype = (select case when target.Order_Date__c < @DeleteDateThreshold THEN 'd' else 'c' END)
