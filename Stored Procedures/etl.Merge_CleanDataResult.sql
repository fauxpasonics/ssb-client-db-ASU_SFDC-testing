...
BEGIN 
	...
	--	remove curly braces from ContactId				DCH 2016-10-20
	update #CD_DataSet
	set ContactId = REPLACE(REPLACE(ContactId,'{',''),'}','')
	;


	--	update the patron records with the clean data results
	update p
	set 
	p.ContactId = 
	case 
...
/*		Removed after initial push to SF

-----	attempt to update Account.ContactId from dbo.Patron__c				DCH 2016-10-20
	update A
	set A.ContactId = B.ContactId
	from dbo.Account A
	join dbo.Patron__c B
	on A.Patron = B.Patron
	and ISNULL(A.ContactId,'00000000-0000-AAAA-0000-000000000000') <> ISNULL(B.ContactId,'00000000-BBBB-0000-0000-000000000000');



-----	attempt to update Account.ContactId from dbo.Patron__c where the Account record has multiple Patron values				DCH 2016-10-20
	
	with firststep as (
		select ContactId
		from dbo.Patron__c
		where ISNULL(ContactId,'00000000-0000-0000-0000-000000000000') <>'00000000-0000-0000-0000-000000000000'
		group by ContactId
		having count(9) > 1
	)
	select A.ContactId, A.Patron,
		ROW_NUMBER() OVER (Partition By A.ContactId Order By A.Patron ASC) as RowNum
	into #MultiplePatrons
	from dbo.Patron__c A
	join firststep B
	on A.ContactId = B.ContactId;

	SET @MaxRowNum = (SELECT MAX(RowNum) from #MultiplePatrons);
	...
	BEGIN
		...
		update A
		set A.Patron = CONCAT(A.Patron
							, CASE WHEN B.Patron IS NOT NULL THEN ', ' ELSE '' END
							, B.Patron)
		from #MultiplePatrons A
		left join #MultiplePatrons B
			on A.ContactId = B.ContactId
			and A.RowNum = 1
			and B.RowNum = @Counter
			and A.Patron <> B.Patron;

		IF @Counter >= @MaxRowNum
			BREAK
		ELSE
			CONTINUE;
	...
	--	select * from #MultiplePatrons where RowNum = 1

	update A
	set A.ContactId = B.ContactId
	from dbo.Account A
	join #MultiplePatrons B
		on A.Patron = B.Patron
		and B.RowNum = 1
		and ISNULL(A.ContactId,'00000000-0000-AAAA-0000-000000000000') <> ISNULL(B.ContactId,'00000000-BBBB-0000-0000-000000000000');


-----	end of code				DCH 2016-10-20
...
*/
	...
	where t1.ContactId is not null and t1.contactid <> '00000000-0000-0000-0000-000000000000'
	GROUP BY t1.ContactId


/*****

DCH on 2016-11-08	-	New patron records are already created in sproc dbo.Merge_Patron__c.  CleanData output is used to update
						existing records, but, otherwise, the information is already there from TI.  Account records, on the other hand,
						have their name, address, etc. information populated solely using CleanData output.  Instead of relying on CleanData
						to populate account records, use information from just-updated or created patron records (which are already updated
						using CleanData output) so that newly-created account records have information from CleanData or TI, but
						they should always have something.  Looking back 2 days might be a bit much for most executions, but it's a safety
						next in a situation where a job failure can't be addressed until the following day (or later).
...
*****/
	...
	into #PatronContact
	from dbo.Patron__c p
--	inner join #CD_DataSet ds on p.Patron = ds.SourceContactId					-- DCH 2016-11-08
	left outer join (
	...
	and p.dbLastUpdated >= DATEADD(DAY,-2,GETDATE())							-- DCH 2016-11-08
	and p.ChangeType = 'c'														-- DCH 2016-11-08
	;
	...
	, p.PatronStatusCode, p.PatronStatus, p.CustomerTypeCode, p.CustomerType, p.CustomerStatus, p.PriorityPtsTix, p.PacCreateDate
	...
	THEN UPDATE 
	SET target.dbLastUpdated = getdate()
	, target.changetype = 'c'
...
-----	set salesforce_id in dbo.Account using Id from dbo.SFDC_Account, which is a list of SFDC Account information pull in full daily			DCH 2016-11-02
	update A
	set A.salesforce_ID = B.Id
	from dbo.Account A
	join dbo.SFDC_Account B
	on A.Patron = B.Patron_ID__c
